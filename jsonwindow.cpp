...
void JSONWindow::readFrom(const QJsonDocument &doc)
{
    if ( !m_pModel ) return;
...
QString JSONWindow::labelForValue(const QJsonValue &val)
{
    switch ( val.type() )
    {
...
void JSONWindow::addJsonArray(const QJsonArray &arr, QStandardItem *item)
{
    if ( !item ) return;
...
void JSONWindow::addJsonObject(const QJsonObject &obj, QStandardItem *item)
{
    if ( !item ) return;
